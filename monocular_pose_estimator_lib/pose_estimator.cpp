...
PoseEstimator::PoseEstimator()
{
  ...
  //---------------------new add
  back_projection_pixel_tolerance_ = 15;//5;
  nearest_neighbour_pixel_tolerance_ = 20;//7;

  detection_threshold_value_ = 140;
  gaussian_sigma_ = 0.6;
  min_blob_area_ = 10;
  max_blob_area_ = 2000;//200
  max_width_height_distortion_ = 0.5;
  max_circular_distortion_ = 0.5;
  roi_border_thickness_ = 20;
  //---------------------new add
}
...
//      ROS_WARN("Initialising using brute-force correspondence search.");
      ...
      //std::cout<<"*****"<<std::endl;//-----能输出

      if (initialise() == 1)//模板匹配
      {
        //std::cout<<"11*****"<<std::endl;//----不能输出
...
void PoseEstimator::calculateImageVectors()
{
  ...
  {
    single_vector(0) = (image_points_(i)(0) - camera_matrix_K_.at<double>(0, 2)) / camera_matrix_K_.at<double>(0, 0);// x/z
    single_vector(1) = (image_points_(i)(1) - camera_matrix_K_.at<double>(1, 2)) / camera_matrix_K_.at<double>(1, 1);// y/z
    single_vector(2) = 1;
