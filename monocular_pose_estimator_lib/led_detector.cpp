                           ...
                           const cv::Mat &camera_matrix_K, const std::vector<double> &camera_distortion_coeffs)
{
  ...
  cv::threshold(image(ROI), bw_image, threshold_value, 255, cv::THRESH_TOZERO);//THRESH_TOZERO---0阈值(if(src(x,y)>threshold src(x,y) else 0)
  ...
  //cv::imshow( "Gaussian_ROI", gaussian_image );
  ...
  //------------------------------  new add
  cv::Mat testImg(image.rows,image.cols,CV_8UC3,cv::Scalar(0,0,0));
  // Draw a circle around the detected LED
  for (int i = 0; i < distorted_points.size(); i++)
  {
    cv::circle(testImg, distorted_points[i], 2, CV_RGB(255, 0, 0), 2);
  }
  imshow("center_points",testImg);
  //-----------------------------  new add
